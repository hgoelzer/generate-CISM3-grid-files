...
% grid dimensions of the 1km g1 base grid
nx_base=1681;
ny_base=2881;
...
% For any resolution but check integer grid numbers
    ...
    if(isaninteger(nx) & isaninteger(ny))
        ...
        agrid(index).LatLonOutputFileName=['grid_CISM3_g0_GrIS_' sprintf('%05d',r*1000) 'm.nc'];
        agrid(index).CDOOutputFileName=['grid_CISM3_g0_GrIS_' sprintf('%05d',r*1000) 'm.txt'];
        agrid(index).xyOutputFileName=['xy_CISM3_g0_GrIS_' sprintf('%05d',r*1000) 'm.nc'];
    else
