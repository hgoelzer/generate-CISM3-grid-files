...
% For any resolution but check integer grid numbers
    ...
    if(isaninteger(nx) & isaninteger(ny))
        ...
        agrid(index).LatLonOutputFileName=['grid_CISM3_g1_AIS_' sprintf('%05d',r*1000) 'm.nc'];
        agrid(index).CDOOutputFileName=['grid_CISM3_g1_AIS_' sprintf('%05d',r*1000) 'm.txt'];
        agrid(index).xyOutputFileName=['xy_CISM3_g1_AIS_' sprintf('%05d',r*1000) 'm.nc'];
    else
