...
% Heiko Goelzer, February 2024 (heig@norceresearch.no)
...
af2_file_name = ['af2_CISM3_g1_AIS_' sprintf('%05d',res*1000) 'm.nc'];
...
% Coordinates
nccreate(af2_file_name,'x','Dimensions',{'x',nx}, 'Datatype','single', 'Format','NETCDF4');
nccreate(af2_file_name,'y','Dimensions',{'y',ny}, 'Datatype','single', 'Format','NETCDF4');
...
% Data
wnc(single(af2),af2_file_name,'af2','1','projection area scale factor',{'x','y'},0,'NETCDF4')
ncwriteatt(af2_file_name,'af2','grid_mapping','mapping')

ncwriteatt(af2_file_name,'/','proj4','+init=epsg:3031')
ncwriteatt(af2_file_name,'/','Description','Area scaling factor (af2) for ISMIP6 Antarctic grid. Multiply with 2D data to correct the projection error. af2=(1/k)^2, where k is the map scale factor. Calculated after Snyder (1987) by Heiko Goelzer, 2024.')
